...
% Model
q = ( -2 * x4 + ( -2 * x7 + ( -2 * x8 + ( -2 * x9 + ( -2 * x10))))) * x1 + ((x5 + (x9 + (x10))) * x2 + (( - x6 + ( - x10)) * x3));
cstr = [(4-x1-x2-x3)/4;
        (6-3*x2-x3)/6;
        (-0.75+2*x1-2*x3+4*x1*x1-4*x1*x2+4*x1*x3+2*x2*x2-2*x2*x3+2*x3*x3)/55.25];

n_semialg =  length(cstr);
    
interval=[[0 2];[0 2];[0 3];repmat([-1 1],nparam,1)];
